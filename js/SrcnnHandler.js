define(['ChannelImage', 'ImagePlane'], (ChannelImage, ImagePlane) => {
  "use strict";
  
  return class SrcnnHandler {
      constructor(json) {
      ...
      static calcRGB(imageR, imageG, imageB, model, scale, progress, phase) {
          let [ planeR, planeG, planeB ] = [imageR, imageG, imageB].map((image) => {
              ...
              return SrcnnHandler.normalize(imgExtra);
          });
          ...
          // init W
          let W = SrcnnHandler.typeW(model);
          ...
          for (let b = 0; b < inputBlocks.length; b++) {
              ...
              for (let l = 0; l < model.length; l++) {
                  ...
                  // convolution
                  outputBlock = SrcnnHandler.convolution(inputBlock, W[l], nOutputPlane, model[l]["bias"]);
          ...
          [ imageR, imageG, imageB ] = outputPlanes.map((outputPlane) => {
              return SrcnnHandler.denormalize(outputPlane);
          });
      ...
      calc(image, width, height, done, progress) {
          ...
          if (this.scale2xModel != null) {
              ...
              [ imageR, imageG, imageB ] = SrcnnHandler.calcRGB(imageR, imageG, imageB, this.scale2xModel, this.scale, progress, 'scale');
